...
exports.loginUser = async (req: express.Request, res: express.Response) => {
  try {
    ...
    const userCheck = await db.UserLogin.findOne({
      where: { email: email, password: password }
    })
    assert(
      userCheck !== null &&
        bcrypt.compare(userCheck.getDataValue('password'), password)
    )
    const userInfo = await db.UserProfile.findOne({
      where: { name: userCheck.getDataValue('name') }
    })
...
exports.deleteCard = async (req: express.Request, res: express.Response) => {
  try {
    ...
    await Wips.updateOne(
      { _id: wipId },
      { $pull: { wip_cards: { _id: cardId } } }
    )
